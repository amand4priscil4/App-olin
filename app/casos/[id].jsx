...
// Tab 4: Vítimas - USANDO O COMPONENTE ListVitimas
...
export default function CaseDetails() {
  ...
  const [showVitimaModal, setShowVitimaModal] = useState(false); // Novo estado para modal de vítimas
  ...
  // Função atualizada para abrir modal de vítimas
  const handleAddVitima = () => {
    setShowVitimaModal(true);
  };

  // Função para quando uma vítima é criada
  const handleVitimaCreated = (novaVitima) => {
    // Atualizar lista de vítimas
    setVitimas(prev => [novaVitima, ...prev]);
    Alert.alert("Sucesso", "Vítima cadastrada com sucesso!");
  };

  // Função para quando uma vítima é pressionada
  const handleVitimaPress = (vitima) => {
    // Navegar para detalhes da vítima ou mostrar ações
    Alert.alert(
      "Vítima Selecionada",
      `${vitima.nome}\nNIC: ${vitima.nic}`,
      [
        ...
        { 
          text: "Ver Detalhes", 
          onPress: () => {
            // Aqui você pode navegar para uma tela de detalhes da vítima
            // router.push(`/vitimas/${vitima._id}`);
            console.log("Navegar para detalhes da vítima:", vitima._id);
          }
        }
      ]
    );
  ...
  const tabsData = [
    ...
    },
    {
      ...
      props: { vitimas, onAddVitima: handleAddVitima, onVitimaPress: handleVitimaPress }
    }
  ];
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#f5f5f5' }}>
      ...
      <ModalNovaVitima
        ...
        casoId={id}
        onVitimaCreated={handleVitimaCreated}
      />
    </View>
  );
}