...
export default function NewCaseModal({ visible, onClose, onSuccess }) {
  ...
  // Tipos de caso disponíveis - CORRIGIDO para coincidir com o backend
  const tiposCaso = [
    ...
    { label: "Exumação", value: "exumação" },
    { label: "Violência Doméstica", value: "violência doméstica" },
    { label: "Avaliação de Idade", value: "avaliação de idade" },
    { label: "Avaliação de Lesões", value: "avaliação de lesões" },
    { label: "Avaliação de Danos Corporais", value: "avaliação de danos corporais" }
  ];
  ...
  const handleSalvarCaso = async () => {
    ...
    try {
      ...
      console.log('User ID do token:', tokenPayload.id);
      ...
      const novoCaso = {
        ...
        status,
        peritoResponsavel: tokenPayload.id, // Campo obrigatório no schema
      ...
      console.log('Dados sendo enviados:', JSON.stringify(novoCaso, null, 2));
